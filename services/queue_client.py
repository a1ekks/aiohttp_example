...
class RabbitMqConfig:
    ...
    def __init__(self,
        ...
        self.management_api_port = management_port

        self.set_url()
        self.set_management_api_url(management_port)
    ...
    def set_management_api_url(self, management_port):
...
async def init_rabbitmq_client_adapter(rabbitmq_config: dict, loop=None) -> QueueClientAbstract:
    ...
    rabbitmq = RabbitMqQueueClient(rmq_config, loop)
    await rabbitmq.init_connection()

    return rabbitmq


class RabbitMqQueueClient(QueueClientAbstract):
    ...
    async def create_queue(self, queue_name: str, ttl_hours=None):
        # is_exists = await self.check_queue_exists(queue_name)
        # if is_exists:
        #     return queue_name
        if ttl_hours:
